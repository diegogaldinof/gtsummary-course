...
.footnote[
There are different ways to use this syntax. Check the [manual](http://www.danieldsjoberg.com/gtsummary/articles/tbl_summary.html#customize-output-1).
]
...
`missing` tells whether to display a row with the number of missing observations. *Default* is `TRUE`.
...
Specifies the decimal digits in the descriptive statistics.

Use: `digits = variable_1 ~ #`, where # sets the amount of digits you want.
...
To determine the amount of decimal digits for **each** descriptive measure (e.g., *n*, mean and SD):
...
Receives **one** categorical variable (e.g., sex/gender).

Descriptive statistics will be computed to each **category** (e.g., male, female).

It's the first step to perform comparison tests between groups.
...
Used to round and format p-values. *Default* is 1 decimal digit

This argument receives a function. You can use the [`gtsummary::style_pvalue()`](http://www.danieldsjoberg.com/gtsummary/reference/style_pvalue.html) function or you can create your own function.
...
- `bold_labels()`: **Bold** the **names** of variables.

- `italicize_labels()`: *Italicize* the *names* of variables.
...
- `bold_levels()`: **Bold** the **categories** of variables.

- `italicize_levels()`: *Italicize* the *categories* of variables.
...
The `gtsummary` package prints the tables in HTML format. We can save them in several formats (e.g., png, svg, pdf, Word).
...
- See the [package website](http://www.danieldsjoberg.com/gtsummary/index.html) to learn:
